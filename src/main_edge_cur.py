...
class CLNode:
    ...
    def data_scheduler(self):
        ...
        weights = torch.zeros(self.difficulty.shape[0])
...
def train(ARG):
    ...
    ARG.device = dev
    clnode = CLNode(alpha=0.8, mode='linear', period=5, total_epoch=120, lambda_0=0.25)
    ...
    for i in range(ARG.epoch):
        ...
        for bnum, st_idx in enumerate(range(0, n, batch_size)):
            ...
            batch_weights = weights[list(subgraph.nodes())].to(dev)
