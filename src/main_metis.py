...
def train(ARG):
    ...
    punish = 1
    for edge in graph.edges():
        graph[edge[0]][edge[1]]["weight"] = 160
    graph.graph['edge_weight_attr'] = 'weight'
    for epochNum in range(ARG.epoch):
        loss_list = []
        num = graph.number_of_edges()
        (cut, parts) = part_graph(graph, ARG.batchNum)
        partition_nodes = [[] for i in range(ARG.batchNum)]
        for i in range(len(parts)):
            partition_nodes[parts[i]].append(i)
        subgraphs = []
        for i in range(ARG.batchNum):
            subgraph = graph.subgraph(partition_nodes[i])
