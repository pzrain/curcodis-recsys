...
class Scheduler:
    ...
    def schedule(self):
        if self.mode == 'linear':
            ...
            threshold = min(1, np.pow(2, np.log2(self.lambda_0) - np.log2(self.lambda_0) * self.epoch / self.curriculum_epoch))
        self.epoch += 1
        return threshold
...
def train(ARG):
    ...
    subgraphs = two_k_partition(graph, ARG.partitionK)
    subgraphs_with_difficulty = []
    for subgraph in subgraphs:
        _, _, I, _, _, _ = calc_difficulty(subgraph)
        subgraphs_with_difficulty.append((subgraph, I))
    subgraphs_with_difficulty.sort(key=operator.itemgetter(1))
    ...
    for epochNum in range(ARG.epoch):
        ...
        subgraphs_cur = []
        for subgraph, difficulty in subgraphs_with_difficulty:
            if difficulty > threshold:
                break
            subgraphs_cur.append(subgraph)
        if len(subgraphs_cur) == 0:
            subgraphs_cur.append(subgraphs_with_difficulty[0][0])
        for subgraph in subgraphs_cur:
            # subgraph = subgraphs_cur[i]
            subgraph_nodes = list(subgraph.nodes())
            x = train_data[subgraph_nodes].to(dev)
            mapping = dict(zip(subgraph_nodes, range(subgraph.number_of_nodes()))) 
            subgraph = nx.relabel_nodes(subgraph, mapping)
        ...
        if (epochNum + 1) % 20 == 0:
            test(ARG, model, graph, train_data, T, dev, epochNum, subgraphs_with_difficulty)
